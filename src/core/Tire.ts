...
const parseRes = (node: TireNode) => {
	...
	if (content) {
		try {
			...
			throw new Error(`The response data of ${node.path} is incorrect. \n Please edit it to meet the JSON format and try again!`)
		}
	}
	return result
}
...
const parseReq = (node: TireNode) => {
	...
	if (content) {
		try {
			...
			throw new Error(`The request form data of ${node.path} is incorrect. \n Please edit it to meet the JSON format and try again!`)
		}
	}	
	return result
}
